...
export default function Create({ clients = [], users = [], prefill_client_id = null }) {
    ...
    return (
        <AuthenticatedLayout title="New requirement">
            <Head title="New requirement" />
            <PageHeader title="New requirement" description="Create a staff-entered client requirement" />
            ...
            >
                ...
                {showIm && (
                    <div className="space-y-3 rounded-lg border border-surface-border p-4">
                        <h3 className="font-display text-sm font-semibold">Influencer marketing</h3>
                        <div>
                            <label className="crm-label">Influencers required</label>
                ...
                {showSmm && (
                    <div className="space-y-3 rounded-lg border border-surface-border p-4">
                        <h3 className="font-display text-sm font-semibold">Social media marketing</h3>
                        <div>
                            <label className="crm-label">Services</label>
                            <div className="mt-1 flex flex-wrap gap-2">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            <div>
                                ...
                                <input className="crm-input" value={data.duration} onChange={(e) => setData('duration', e.target.value)} />
                            </div>
                            <div>
                                <label className="crm-label">Posting frequency</label>
                                <input className="crm-input" value={data.posting_frequency} onChange={(e) => setData('posting_frequency', e.target.value)} />
                            </div>
                        </div>
                        <div>
                ...
                <div>
                    ...
                    <textarea className="crm-input min-h-[80px]" value={data.internal_notes} onChange={(e) => setData('internal_notes', e.target.value)} />
                </div>

                <div className="flex justify-end">
                    <button type="submit" className="crm-btn-primary" disabled={processing}>Create</button>
                </div>
            </form>
        </AuthenticatedLayout>
    );
