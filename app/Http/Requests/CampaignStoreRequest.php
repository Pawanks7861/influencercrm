...
class CampaignStoreRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            $type = CampaignType::tryFrom((string) $this->input('campaign_type'));

            if ($type?->includesInfluencers() && empty($this->input('influencers'))) {
                $validator->errors()->add('influencers', 'Add at least one influencer for this campaign type.');
            }

            if ($type?->includesSocialMedia() && empty($this->input('deliverables'))) {
